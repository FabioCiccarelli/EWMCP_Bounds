...
// TSM time limit variables (defined here, declared extern in header)
int TSM_TIME_LIMIT_HIT = 0;
chrono::steady_clock::time_point TSM_wall_start;
double TSM_TIME_LIMIT_SEC = 1e30;

void set_TSM_time_limit(double seconds)
{
	TSM_TIME_LIMIT_SEC = seconds;
	TSM_TIME_LIMIT_HIT = 0;
	TSM_wall_start = chrono::steady_clock::now();
}

bool TSM_check_time_limit()
{
	double elapsed = chrono::duration<double>(
		chrono::steady_clock::now() - TSM_wall_start).count();
	if (elapsed >= TSM_TIME_LIMIT_SEC) {
		TSM_TIME_LIMIT_HIT = 1;
		return true;
	}
	return false;
}
...
int init_TSM(instance *inst) 
{
	...
	for(node = 1; node <= inst->G->nnodes; node++)
	{
		weight = (long long) inst->VERTEX_WEIGHTS_int[node-1];
		Top_Weight[node] = weight;
		Node_Weight[node] = Top_Weight[node];
		if (node > max_node)
			max_node = node;
	...
	{
		l_node = inst->G->tail[e]+1;
		r_node = inst->G->head[e]+1;
	...
	// Force LARGE_WEIGHT = TRUE to avoid the weight-based first-level reduction
	// (reduce_first_level_subgraphs_by_weight), which has a bug when many nodes
	// have weight 0: the condition cur_weight == total_weight does not imply that
	// the remaining nodes form a clique. The degree-based reduction
	// (reduce_first_level_subgraphs_by_degree) is always correct.
	LARGE_WEIGHT = TRUE;
...
static void search_maxclique(int cutoff, int using_init_clique) {
	...
	while (CURSOR> 0) {
		// Periodic time limit check (every 1024 branches)
		if ((BRANCHING_COUNT & 0x3FF) == 0 && BRANCHING_COUNT > 0) {
			if (TSM_check_time_limit()) break;
		}
	...
	if (INIT_CLQ_WEIGHT < MAX_CLQ_WEIGHT) {
		...
		for (i = 0; i < MAX_CLQ_SIZE; i++) {
			...
			for (j = i + 1; j < MAX_CLQ_SIZE; j++) {
				...
				int a = node1 > node2 ? node1 : node2;
				int b = node1 > node2 ? node2 : node1;
				if (is_adjacent(a, b) != TRUE) {
					return;
				}
			}
		}
...
static void reduce_instance_for_degree() {
	...
	//printf("I the reduced graph: #node %d #edge %d #density %9.8f \n", NB_NODE, NB_EDGE, ((float) NB_EDGE * 2 / NB_NODE / (NB_NODE - 1)));
	N0_A = NB_NODE;
	D0_A = (NB_NODE > 1) ? ((float) NB_EDGE * 2 / NB_NODE / (NB_NODE - 1)) : 0.0;
	//printf("I the largest subgraph is %d\n", MAX_SUBGRAPH_SIZE);
	INIT_TIME = get_utime() - READ_TIME;
...
int TSM_MWC(instance *inst)
{
	...
	clear_structures();
	if (build_TSM_graph_instance(inst)) {
		if (INIT_ORDERING == 3) {
	...
	//printf(">> |V| %d |E| %d D %0.8lf max_weight %lld branches %d read_time %4.2lf init_time %4.2lf search_time %4.2lf total_time %4.2lf \n",
	// NB_NODE_O, NB_EDGE_O,
	...
	if (INIT_CLQ_WEIGHT < MAX_CLQ_WEIGHT || SEARCH_UPDATED_CLIQUE) 
	{
		for (int i = 0; i < MAX_CLQ_SIZE; i++) 
		{
			int orig = Old_Name[MaxCLQ_Stack[i]] - 1;
			inst->MWCP_X[orig] = 1.0;
		}
	} 
	else 
	{
		for (int i = 0; i < MAX_CLQ_SIZE; i++) 
		{
			inst->MWCP_X[MaxCLQ_Stack[i] - 1] = 1.0;
		}
	}


	return (double) (MAX_CLQ_WEIGHT/MULT);
}


void free_TSM()
{
	// Note: free_block() is already called during reduce_instance_for_* 
	// which frees Init_Adj_List and block allocations. After that, 
	// Node_Neibors points into Adj_List, and BLOCK_COUNT is reset internally.

	// Free Adj_List (allocated in reduce_instance_for_*)
	if (Adj_List != NULL) { free(Adj_List); Adj_List = NULL; }

	// Free Node_Neibors array (the pointer array, not the data it pointed to)
	if (Node_Neibors != NULL) { free(Node_Neibors); Node_Neibors = NULL; }

	// Free MaxCLQ_Stack
	if (MaxCLQ_Stack != NULL) { free(MaxCLQ_Stack); MaxCLQ_Stack = NULL; }

	// Free allocate_memory allocations
	if (Second_Name != NULL) { free(Second_Name); Second_Name = NULL; }
	if (iSET != NULL) {
		if (iSET[0] != NULL) { free(iSET[0]); iSET[0] = NULL; }
		free(iSET); iSET = NULL;
	}
	if (iSET_Size != NULL) { free(iSET_Size); iSET_Size = NULL; }
	if (iSET_Weight != NULL) { free(iSET_Weight); iSET_Weight = NULL; }
	if (iSET_Tested != NULL) { free(iSET_Tested); iSET_Tested = NULL; }
	if (Clique_Stack != NULL) { free(Clique_Stack); Clique_Stack = NULL; }
#ifdef MaxSAT
	if (IS != NULL) { free(IS); IS = NULL; }
	if (UNIT_STACK != NULL) { free(UNIT_STACK); UNIT_STACK = NULL; }
	if (NEW_UNIT_STACK != NULL) { free(NEW_UNIT_STACK); NEW_UNIT_STACK = NULL; }
	if (REASON_STACK != NULL) { free(REASON_STACK); REASON_STACK = NULL; }
	if (TOPK_REDUCED_STACK != NULL) { free(TOPK_REDUCED_STACK); TOPK_REDUCED_STACK = NULL; }
#endif
	if (Adj_Matrix != NULL) { free(Adj_Matrix); Adj_Matrix = NULL; }
	if (Old_Name != NULL) { free(Old_Name); Old_Name = NULL; }

	// Reset key static counters
	...
	FIXED_NODES_STACK_fill_pointer = 0;
	SATISFIED_iSETS_STACK_fill_pointer = 0;
	TOPK_REDUCED_STACK_fill_pointer = 0;
	iSET_COUNT = 0;
	iSET_TOTAL_WEIGHT = 0;
	NEW_NODE_IDX = 0;
	MAX_OLD_NODE = 0;
